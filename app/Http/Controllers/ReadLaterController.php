...
class ReadLaterController extends Controller
{
    ...
    public function index(){
      ...
      $data['articles'] = DB::table('articles as a')->select(
                    ...
                    DB::raw('(SELECT count(uc.article_id) from urgent_cab uc WHERE a.id = rl.article_id='.session('id').') as u_urgent'),
                    DB::raw('(SELECT count(article_id) from likes l WHERE a.id=l.article_id and rl.user_id='.session('id').') as u_likes'),
                    DB::raw('(SELECT count(ra.article_id) from readalready ra WHERE a.id = ra.article_id and rl.user_id='.session('id').') as u_read'),
                    DB::raw('(SELECT count(rl.article_id) from readlater rl WHERE a.id=rl.article_id and rl.user_id='.session('id').') as u_unread' )
                  
                  )
                ->leftjoin('readlater as rl', 'rl.article_id', '=', 'a.id')
        ...
        return view('je_lis_plus_tard', $data);
    }

    public function readlater_search(Request $request){
      ...
      $data['articles'] = DB::table('articles as a')->select(
          ...
          DB::raw('(SELECT count(ra.article_id) from readalready ra WHERE a.id = ra.article_id) as t_read'),
          DB::raw('(SELECT count(rl.article_id) from readlater rl WHERE a.id=rl.article_id) as t_unread'),
          DB::raw('(SELECT count(uc.article_id) from urgent_cab uc WHERE a.id = rl.article_id and rl.user_id='.session('id').') as u_urgent'),
          DB::raw('(SELECT count(article_id) from likes l WHERE a.id=l.article_id and rl.user_id='.session('id').') as u_likes'),
          DB::raw('(SELECT count(ra.article_id) from readalready ra WHERE a.id = ra.article_id and rl.user_id='.session('id').') as u_read'),
          DB::raw('(SELECT count(rl.article_id) from readlater rl WHERE a.id=rl.article_id and rl.user_id='.session('id').') as u_unread' )
