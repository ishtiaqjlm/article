...
use App\Models\Days;
use DB;
class ReadAlreadyController extends Controller
{
    //
    public function index(){
        ...
        if(session('id') ==""){
         ...
         $data['articles'] = DB::table('articles as a')->select(
                    ...
                    DB::raw('(SELECT count(uc.article_id) from urgent_cab uc WHERE a.id = ra.article_id='.session('id').') as u_urgent'),
                    DB::raw('(SELECT count(article_id) from likes l WHERE a.id=l.article_id and ra.user_id='.session('id').') as u_likes'),
                    DB::raw('(SELECT count(ra.article_id) from readalready ra WHERE a.id = ra.article_id and ra.user_id='.session('id').') as u_read'),
                    DB::raw('(SELECT count(rl.article_id) from readlater rl WHERE a.id=rl.article_id and ra.user_id='.session('id').') as u_unread' )
                  
                  )
                ->leftjoin('readalready as ra', 'ra.article_id', '=', 'a.id')
                ...
                return view('j_ai_deja_lu', $data);
    }

   public function search_read_already(Request $request){
